                        ...
                        <div class="flex items-center">
                          ...
                          <Button
                            ...
                            @click="
                              router.push({
                                name: 'Contact',
                                params: { contactId: contact.name }, // Ensure contactId is passed correctly
                              })
                            "
                          />
                          <Button
...
const { doctypeMeta } = getMeta('Gamma Proposal')
...
watch(error, (err) => {
  if (err) {
    console.error("Error in document watcher:", err);
    errorTitle.value = __(
...
const breadcrumbs = computed(() => {
  if (!props.GammaId) {
    console.error("GammaId is missing or undefined");
    return [];
  }
  ...
  items.push({
    label: title.value,
    route: { name: 'Gamma', params: { GammaId: props.GammaId } },
  });
  return items;
})
    ...
    section.columns[0].fields.forEach((field) => {
      if (field.fieldname == 'organization') {
        ...
        field.link = (org) =>
          router.push({
            name: 'Organization',
            params: { organizationId: org }, // Ensure organizationId is passed correctly
          })
      }
    })
  })
