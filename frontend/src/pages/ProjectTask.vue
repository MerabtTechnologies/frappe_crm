...
const breadcrumbs = computed(() => {
  let items = [{ label: __('Project Tasks'), route: { name: 'Project Tasks' } }]

  if (route.query.view || route.query.viewType) {
    let view = getView(route.query.view, route.query.viewType, 'Project Task')
    if (view) {
      items.push({
        label: __(view.label),
        icon: view.icon,
  ...
  items.push({
    label: title.value,
    route: { name: 'Project Task', params: { taskId: props.taskId } },
  })
  return items
})

const title = computed(() => {
  let t = doctypeMeta['Smart Task']?.title_field || 'name'
  return doc.value?.[t] || props.taskId
})
...
const sections = createResource({
  ...
  cache: ['sidePanelSections', 'Smart Task'],
  params: { doctype: 'Smart Task' },
...
function beforeStatusChange(data) {
  ...
  // TODO remove above lines solve saving error
  document.save.submit(null, {
      onSuccess: () => reloadAssignees(data),
    })
...
function statusColor(status) {
  ...
  if (status === 'Open') return 'text-grey-600'
  if( status === 'Pending Review' || status === 'Planning') return 'text-yellow-600'
