<template>
  <LayoutHeader>
    <template #left-header>
      <ViewBreadcrumbs v-model="viewControls" routeName="Gammas" />
    </template>
  ...
  <div v-else-if="deals.data" class="flex h-full items-center justify-center">
    ...
    >
      ...
      <span>{{ __('No {0} Found', [__('Gammas')]) }}</span>
      <Button
        :label="__('Create')"
        iconLeft="plus"
        @click="showDealModal = true"
...
// import GammaProposalListVIew from '@/components/ListViews/GammaProposalListVIew.vue'
...
function showNote(name) {
  showModal({
    ...
    title: 'Note',
    defaults: {
      reference_doctype: 'Gamma Proposal',
