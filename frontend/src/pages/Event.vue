...
const breadcrumbs = computed(() => {
  let items = [{ label: __('Event'), route: { name: 'Events' } }]

  if (route.query.view || route.query.viewType) {
    let view = getView(route.query.view, route.query.viewType, 'Event')
    if (view) {
      items.push({
        label: __(view.label),
        icon: view.icon,
  ...
  items.push({
    label: title.value,
    route: { name: 'Event', params: { eventId: props.eventId } },
  })
  return items
})

const title = computed(() => {
  let t = doctypeMeta['Event']?.title_field || 'name'
...
const dealContacts = createResource({
  ...
  params: { name: props.eventId },
  cache: ['deal_contacts', props.eventId],
  transform: (data) => {
...
function beforeStatusChange(data) {
  ...
  // set mandatory field for event reference
  // if (
