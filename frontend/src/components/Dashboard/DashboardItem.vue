<!-- <template>
  <div class="h-full w-full">
    ...
    <div
      v-else-if="item.type == 'line_chart'"
      class="h-full w-full rounded-md bg-surface-white shadow overflow-hidden"
    >
      <LineChart v-if="item.data" :config="item.data" />
  </div>
</template>
<script setup>
import { AxisChart, DonutChart, NumberChart, Tooltip, LineChart } from 'frappe-ui'
...
<template>
  <div class="h-full w-full">
    ...
    >
      ...
      <div
    v-else-if="item.type == 'horizontal_bar_chart'"
    class="h-full w-full rounded-md bg-surface-white shadow overflow-hidden"
  >
    <AxisChart v-if="item.data?.config" :config="item.data.config" />
