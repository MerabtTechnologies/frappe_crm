    ...
    <div v-for="(task, i) in tasks" :key="task.name">
      ...
      > -->
        <div class="flex flex-1 flex-col gap-1.5 text-base truncate">
          <div class="font-medium text-ink-gray-9 truncate" @click="modalRef.showTask(task)">
            {{ task.title }}
          </div>
        ...
        <div class="flex items-center gap-1">
          <Dropdown
            :options="projectTaskStatusOptions(modalRef.updateTaskStatus, task)"
            @click.stop
          >
            <Button
...
import { formatDate, projectTaskStatusOptions } from '@/utils'
