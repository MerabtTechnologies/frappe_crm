...
const { document: deal, triggerOnBeforeCreate } = useDocument('Smart Project')
...
const tabs = createResource({
  ...
  cache: ['QuickEntry', 'Smart Project'],
  params: { doctype: 'Smart Project', type: 'Quick Entry' },
  auto: true,
  transform: (_tabs) => {
...
async function createDeal() {
  ...
  createResource({
    ...
    params: { args: deal.doc, doctype: 'Smart Project' },
    auto: true,
    validate() {
      error.value = null
      if (!deal.doc.status) {
    ...
    onSuccess(name) {
      ...
      show.value = false
      router.push({ name: 'Project', params: { projectId: name } })
    },
    onError(err) {
      isDealCreating.value = false
      if (!err.messages) {
...
function openQuickEntryModal() {
  ...
  quickEntryProps.value = { doctype: 'Smart Project' }
