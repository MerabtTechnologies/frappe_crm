...
const { document: deal, triggerOnBeforeCreate } = useDocument('Smart Timesheet')
...
const tabs = createResource({
  ...
  cache: ['QuickEntry', 'Smart Timesheet'],
  params: { doctype: 'Smart Timesheet', type: 'Quick Entry' },
  auto: true,
  transform: (_tabs) => {
...
async function createDeal() {
  ...
  createResource({
    ...
    params: { args: deal.doc, doctype: 'Smart Timesheet' },
    auto: true,
    validate() {
      error.value = null
    ...
    onSuccess(name) {
      capture('smart_timesheet_created')
      isDealCreating.value = false
      show.value = false
      router.push({ name: 'Smart Timesheet', params: { timesheetId: name } })
    },
    onError(err) {
      isDealCreating.value = false
      if (!err.messages) {
...
function openQuickEntryModal() {
  ...
  quickEntryProps.value = { doctype: 'Smart Timesheet ' }
