...
const { document: deal, triggerOnBeforeCreate } = useDocument('Event')
// const { document: deal } = useDocument('Event')
...
async function createDeal() {
  ...
  createResource({
    ...
    params: { args: deal.doc, doctype: 'Event' },
    auto: true,
    validate() {
      error.value = null
      if (!deal.doc.reference_docname || !deal.doc.reference_doctype) {
        error.value = __('Please select a Lead to create Event against.')
        return error.value
      }
    ...
    onSuccess(name) {
      ...
      show.value = false
      router.push({ name: 'Event', params: { eventId: name } })
    },
    onError(err) {
      isDealCreating.value = false
      if (!err.messages) {
...
function openQuickEntryModal() {
  ...
  quickEntryProps.value = { doctype: 'Event' }
  nextTick(() => (show.value = false))
}

onMounted(() => {
  deal.doc.reference_doctype = 'CRM Lead'
  ...
  //         column.fields?.forEach(field => {
  //           if (field.fieldtype === 'Table') {
  //             if (!Array.isArray(deal.doc[field.fieldname])) {
  //               deal.doc[field.fieldname] = []
  //             }
  //           }
  //         })
