        ...
        <div class="mb-5 flex items-center justify-between">
          <div>
            <h3 class="text-2xl font-semibold leading-6 text-ink-gray-9">
              {{ __('Create Employee Date Request') }}
            </h3>
          </div>
...
const { document: deal, triggerOnBeforeCreate } = useDocument('Employee Date Request')
...
const tabs = createResource({
  ...
  cache: ['QuickEntry', 'Employee Date Request'],
  params: { doctype: 'Employee Date Request', type: 'Quick Entry' },
  auto: true,
  transform: (_tabs) => {
...
async function createDeal() {
  ...
  createResource({
    ...
    params: { args: deal.doc, doctype: 'Employee Date Request' },
    auto: true,
    validate() {
      error.value = null
